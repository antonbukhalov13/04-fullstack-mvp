...
Goal: Исправить пропадание серой линии-разделителя между секциями «Вы заранее знаете все условия» и «О компании» на ширине экрана меньше 1024px

Prompt: Между секциями Вы заранее знаете все условия и О компании не хватает серой линии при ширине экрана меньше чем 1024

Result: frontend/src/app/page.tsx — оба разделителя между секциями (после TransparentTerms и после ForBusiness) изменены с h-px w-full bg-slate-300 на h-[2px] w-full shrink-0 bg-slate-300. Причина пропадания — 1px-линия без shrink-0 может схлопываться до 0px на узких вьюпортах или при нецелочисленном масштабе браузера; увеличение толщины до 2px и защита от сжатия делают линию видимой на всех ширинах.

Used as-is / edited manually / rejected: used as-is

What I learned: h-px (1px) — не всегда надёжная высота для декоративных разделителей: при дробном zoom/scale браузера или в определённых layout-контекстах она может визуально «исчезать». h-[2px] + shrink-0 — более устойчивый паттерн для тонких горизонтальных линий-разделителей между секциями.
